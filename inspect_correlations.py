...
def calculate_spearman_correlation(target_data: pd.DataFrame, sensor_data: pd.DataFrame) -> Optional[float]:
    ...
    merged = pd.merge(target_data, sensor_data, on='time', suffixes=('_target', '_sensor'))
...
def analyze_correlations(data_dir: str, building: str, target_sensor: str, resample_freq: str = '1min') -> Dict[str, float]:
    """
    ...
    for sensor_id in building_sensors:
        ...
        correlation = calculate_spearman_correlation(target_resampled, sensor_resampled)
